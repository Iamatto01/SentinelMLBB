...
const STRATEGY_CONFIG: Record<string, { label: string; emoji: string; color: string; bg: string }> = {
  "high-ground":          { label: "High Ground",        emoji: "🏰", color: "text-yellow-650 dark:text-yellow-400",  bg: "bg-yellow-50 dark:bg-yellow-500/10 border-yellow-200 dark:border-yellow-500/20" },
...
const CC_CONFIG: Record<CCTag, { label: string; emoji: string; color: string; bg: string }> = {
  ...
  "no-cc":    { label: "No CC",    emoji: "➖", color: "text-neutral-650 dark:text-neutral-400",  bg: "bg-neutral-50 dark:bg-neutral-500/10 border-neutral-200 dark:border-neutral-700" },
};
...
export default function HeroesPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (apiData?.data?.records) {
            ...
            // Index API records once so enrichment is O(heroes + records) instead of a
            // linear .find() per hero.
            const recordsByMlbbId = new Map<number, any>();
            for (const record of apiData.data.records) {
              const heroId = record?.data?.hero_id;
              if (typeof heroId === "number" && !recordsByMlbbId.has(heroId)) {
                recordsByMlbbId.set(heroId, record);
              }
            }
            ...
            const enriched = merged.map(hero => {
              ...
              const apiRecord = recordsByMlbbId.get(hero.mlbbId);
  ...
  const allTagsByCategory = useCallback((cat: "playstyle" | "strategy") => {
  ...
  const handleDeleteTag = (tagId: string) => {
    ...
    // Remove from heroes' lists. Read the overrides once, mutate in memory, write once —
    // previously this parsed and re-serialized localStorage for every affected hero.
    const savedHeroes = localStorage.getItem("sentinel_custom_heroes");
    const parsedHeroes = savedHeroes ? JSON.parse(savedHeroes) : {};
    ...
    if (overridesChanged) {
      localStorage.setItem("sentinel_custom_heroes", JSON.stringify(parsedHeroes));
    }
